...
def release_notes(name, commit, root=ROOT):
    if not VERSION_PATTERN.fullmatch(name) or not re.fullmatch(r'[0-9a-f]{40}', commit):
        raise ReleaseError('Invalid release notes version or source commit')
    notes_file = root / 'docs/release-notes' / f'{name}.md'
    if not notes_file.is_file() or not (highlights := notes_file.read_text().strip()):
        raise ReleaseError(f'Missing release notes: docs/release-notes/{name}.md')
    return (f'# Cruise Tune {name}\n\n{highlights}\n\n'
            f'<!-- cruise-tune-release:v{name}:{commit} -->\n')
...
def package(tag, root=ROOT, environ=os.environ):
    ...
    notes = release_notes(current['name'], commit, root)
    out = root / 'dist'
