...
夸克网页下载接口两次成功返回空文件列表时，才将队列中的该文件判为云端失效并按实际播放顺序跳过；首次空结果会重新确认。下载地址返回 404 时先重新解析文件标识和地址一次，单独的 404 不作为删除证据。已完整缓存的歌曲仍优先从本地播放。失效项保留在原队列并标记，重新从曲库建立队列后清除标记；网络、登录、权限及未确认错误不会自动跳歌。
