...
应用启动恢复队列后（包括开机启动），会将当前待播放曲目以暂停状态同步到仪表，不为此准备音频、发起网络读取或自动播放。播放器保持 IDLE 时仍保留该恢复曲目的同步资格，接收器稍晚就绪或屏幕稍晚点亮后可补发；成功发送后，定时刷新不会重复发送相同快照，手动“重新检查并发送”可再次发送。开始准备音频或播放、切换曲目或出现播放错误后，结束启动同步阶段。

通话或其它音频占用、熄屏、播放器错误和当前曲目未知时，控制器会失效当前发布状态并取消尚未完成的发送重试。启动恢复曲目仍然有效时，屏幕点亮且其它音频释放后可重新同步暂停快照；这不会恢复播放。
...
同一首歌的播放进度最多约每 1.8 秒发送一次；普通播放状态合并等待 200 ms，避免切歌和播放器回调连续到达时发送中间状态。发送失败时，临时错误按 2、5、15 秒最多重试三次；启动暂停快照和清理状态同样受有限重试约束，定时刷新不会重置重试次数。
