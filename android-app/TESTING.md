...
读取过流式缓存的网盘歌曲发生文件解析错误时，自动绕过缓存重读一次。必须观察到实际播放位置前进后，才下载完整新副本并替换该歌曲的流式缓存；不清空其他歌曲缓存，不自动删除已保留的离线音乐。新副本下载不完整、文件版本变化、空间不足或替换前取消时保留旧缓存。此流程不是整文件哈希校验，也不将播放停在 0 秒直接认定为缓存损坏。

重读后再次出现明确文件错误时跳过当前歌曲；单曲循环不会阻止这次跳过，列表循环中一轮队列均失败则停止。无进度、网络等待、登录失效及权限错误不按坏歌连续跳过。用户暂停、切歌、熄屏或移除来源时取消未完成的恢复。

本机测试使用合成字节、临时文件和真实 Media3 解析器，覆盖错误分类、一次恢复预算、循环边界、缓存读取路径、取消、完整替换及离线缓存隔离。`PlaybackOrderDeviceTest` 另直接读取会话播放器的位置（不是控制器估算值），在随机／顺序切换和显式重试后连续采样，检查播放持续前进，且切换播放顺序不进入停止或重新缓冲状态。设备用例仍仅允许运行在空曲库、空队列的独立验证包中，必须实际报告 `OK (1 test)`。


## 0.5.20 恢复中的网络重试与超时误跳歌

修复 82daae4 审查发现的两项问题：缓存旁路读取不再禁用网络重试；临时网络错误若直接进入播放器错误回调，也会保留恢复状态并安排可取消的退避重载。移除恢复 45 秒后的自动跳歌及 INTERNET 能力判断；断网时发现疑似缓存问题同样进入恢复读取，等待网络恢复。只有明确的文件错误才进入失败跳过流程，未知错误不自动跳过。

完整 209 项主机测试（零失败、零错误）、lintRelease 与签名 Release 构建通过。新增 RecoveryNetworkTest 覆盖：

- 真实 Media3 解析器读取截断的合成 FLAC 缓存，触发旁路恢复；模拟两次 HTTP 网络超时后，读取新音频并进入 READY，曲目与队列不变。
- 直接上报的终止网络错误会延迟执行重新加载，保留原曲目与位置。
- 离线、未验证网络和已验证网络下，模拟时间等待 90 秒均不跳歌、不停止原播放意图。
- 暂停、切歌和熄屏取消待执行的重试，取消后不重新打开数据源。
- 登录失效和未知读取错误不按坏歌跳过，也不进行无效自动重试。

这些网络失败使用合成上游和 Robolectric 播放器验证，没有操作真实账号或损坏实际音频。既有坏歌预算、列表循环终止和单曲循环跳过测试继续通过。

BlueStacks Android 13 / API 33 / ARM64 已保留数据覆盖安装 0.5.20（versionCode 29、同签名、非调试 APK）。更新前为 184 首曲库且未选择当前曲目，更新后曲库保留。通过原来源开始正常播放，画面进度推进，随后暂停于约 1:31。此为正常安装、启动与播放界面复验，没有在模拟器中人为破坏缓存或重演断网流程，不代表完成实车与扬声器音质验收。

尺寸 1920×1080、密度 320、字体 1.0 与旋转设置均保持测试前值；保持调试开启。本地验证与已安装 APK SHA-256：`597ade7c52cc14fbe0637eb579c21a789f9ea02ae479a7bd5d75ed9b374e5f6d`。该摘要对应发布前本地构建；云端发布包的校验值以对应 Release 的 SHA256SUMS 为准。
## 无报错的播放停滞

完整流式缓存对应的歌曲在 READY／BUFFERING 状态下约 10 秒没有实际位置进展时，也进入同一次自动重读流程；状态反复切换不会刷新这个期限。初次停滞超时错误同样接入此流程，不将它直接判定为文件损坏。未完整的网络缓冲、暂停、音频焦点受限及熄屏不积累此停滞时间；手动切歌或调整进度后重新计时。旁路重读中的网络等待不触发新的停滞重读或跳歌；明确文件错误才消耗失败跳过预算，保留 0.5.20 的持续网络重试与无超时跳歌行为。

本机回归模拟每 250 毫秒切换一次 READY／BUFFERING，验证内置检测未报错时应用仍能识别零进度；服务测试覆盖完整缓存无报错停滞、重读后仍无进展、下一首后返回原歌曲、断网不清缓存以及不完整网络缓冲不被误处理。模拟状态与合成数据不能代替实车音频输出验收。

状态切换与播放服务回归同时覆盖 API 28（Android 9）和 API 33。这里验证的是对应 API 环境下的应用／Media3 逻辑，不模拟车机厂商的音频驱动，不能将通过结果等同于实车兼容性确认。

播放器依赖统一更新到 Media3 1.11.0，包含音频瞬态欠载时的就绪状态防抖，以及解码器接收输入前不应执行 flush 的兼容修复，见 [官方版本说明](https://developer.android.com/jetpack/androidx/releases/media3#1.11.0)。这些上游缺陷不局限于 Android 9，也不能仅凭类似症状认定实车属于同一问题。

本机测试使用 Robolectric 4.15.1，其 AudioTrack 模拟已修复 API 28 调用 API 29 专用帧大小方法的问题；增加同步位置读取用例，避免后台播放线程异常被测试的 READY 断言遗漏。该测试依赖不进入应用 APK。

网络恢复测试注入终止错误前，等待已有播放器命令完成确认；仅清空主线程消息不足以保证播放线程已处理 seek/play。定向回归用线程屏障构造未确认命令，验证测试不会把注入错误交给随后抵达的正常状态覆盖。此修正只涉及测试同步，生产代码的过期错误保护保持不变。


## 0.5.24 报错后切歌与缓存读取

定位和修复网盘终止错误后，手动上一首／下一首只移动索引、没有重新 prepare 的问题。修复在服务层执行，保留暂停意图与熄屏限制；读取顺序继续为离线缓存、流式缓存、缺失部分的网盘请求。

原始合成回归在 0.5.23 的 API 28／33 下共有 8 个实例，6 个报错后切歌实例失败、2 个直接缓存播放对照通过；修复后原始实例全部通过。扩展 CachedTrackSelectionTest 共 16 个实例，覆盖连续三首完整缓存、离线层且账号失效、暂停、上一首、快速连点、熄屏及未开始队列。完整缓存歌曲的网络读取次数为零。网络上游会始终拒绝读取，测试未刷新目录或重新授权。

完整 259 项主机测试（零失败、零错误）、lintRelease 与签名 Release 构建通过。0.5.24 / versionCode 33 本地 APK SHA-256：`c69dbcbd3093fa8cede8bc76be5d6c3460e1323b03281b721096f848acf4ade4`，签名与既有版本一致。尚未发布，也未在车机执行本轮复验；初始夸克响应的具体 HTTP 原因没有现场日志。详见 [定位记录](../docs/diagnostics/cached-track-after-error.md)。


## 0.5.25 缓存、凭证与重试审查修复

四项审查问题已落地：重建独立重试并继续后 3 首预缓存；网页登录按加密凭证修订原子写回；完整缓存使用匹配版本的已知长度终止读取；空间预算计入已有缓存且释放空间后可恢复。包含 0.5.24 的报错后切歌修复。

最终 287 项主机测试（零失败、零错误）、lintRelease、同签名 Release 构建通过。新增回归验证多次网络／空间失败后恢复、缓存修复期间后 3 首继续、永久错误不循环、熄屏取消、Cookie 迟到／并发／断开／同文本重登、无效 Cookie 不覆盖、完整缓存无总长元数据时的 EOF、旧队列版本、部分缓存续读、重启保留缓存，以及中途触及存储预留时音频字节仍能完整传递。

修复包 0.5.25 / versionCode 34，非调试 APK，证书与既有版本一致。SHA-256：`646f7238426d864d295e227e6ae726b014a36c4471ba25558d24d243c9ba5985`。当前没有连接的 BlueStacks，未进行本轮设备安装或实车运行；没有修改真实账号、目录或音乐缓存，未创建发布标签。完整结果见 [审查修复记录](../docs/reviews/playback-cache-retry-2026-09-15.md)。
