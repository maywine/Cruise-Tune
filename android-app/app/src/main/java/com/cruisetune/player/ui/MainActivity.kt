...
class MainActivity : CruiseActivity() {
    ...
    private var preferQueueOnOpen = true
    ...
    private val localDirectory = registerForActivityResult(ActivityResultContracts.OpenDocumentTree()) { uri ->
        ...
        app.scope.launch {
            ...
            selectedSource = source.id; showList(false)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        preferQueueOnOpen = savedInstanceState?.getBoolean("preferQueueOnOpen", false) ?: true
    ...
    private fun buildScreen() {
        ...
        val spec = PlayerLayoutSpec.forWindow(config.screenWidthDp, BottomClearance.contentHeightDp(this), config.fontScale)
        ...
        queueTab=TouchButton(this,"队列").apply{minHeight=dp(tabHeight);textSize=20f;setPadding(dp(8),0,dp(8),0);maxLines=1;setOnClickListener{showList(true)}}
        libraryTab=TouchButton(this,"曲库").apply{minHeight=dp(tabHeight);textSize=20f;setPadding(dp(8),0,dp(8),0);maxLines=1;setOnClickListener{showList(false)}}
        ...
        adapter=TrackAdapter(compact){track ->
            ...
            else{showList(true);command(PlaybackService.PLAY_TRACK,Bundle().apply{putString("trackId",track.id);selectedSource?.let{putString("sourceId",it)}})}
        ...
        fun addFooter(button:TouchButton){button.minHeight=dp(76);button.setPadding(dp(8),0,dp(8),0);row.addView(button,LinearLayout.LayoutParams(0,dp(76),1f).apply{if(row.childCount>0)marginStart=dp(8)})}
        ...
        play=TouchButton(this,if(library.tracks.isEmpty())PlaybackAction.ADD.label else PlaybackAction.PLAY.label,true).apply{id=R.id.player_play;setOnClickListener{
            ...
            if(c.playerError != null) {
                ...
                else { showList(true); command(PlaybackService.RETRY); askNotificationPermission() }
            }
            else if(c.isPlaying||c.playWhenReady)c.pause()
            else if(c.mediaItemCount>0){showList(true);if(c.playbackState==Player.STATE_ENDED)c.seekTo(0);c.prepare();c.play();askNotificationPermission()}
            else if(library.tracks.isEmpty())showSources()
            else libraryTracks().firstOrNull()?.let{showList(true);command(PlaybackService.PLAY_TRACK,Bundle().apply{putString("trackId",it.id);selectedSource?.let{source->putString("sourceId",source)}});askNotificationPermission()}
        }}
        updatePlaybackAction(if(library.tracks.isEmpty())PlaybackAction.ADD else PlaybackAction.PLAY)
        addFooter(previous);addFooter(play);addFooter(next)
    ...
    private fun updatePlaybackAction(action:PlaybackAction) {
        ...
        val config=resources.configuration
        val compact=PlayerLayoutSpec.forWindow(config.screenWidthDp,BottomClearance.contentHeightDp(this),config.fontScale).compact
    ...
    private fun showList(queue: Boolean) {
        // An explicit tab/source/play action wins over delayed queue restoration.
        preferQueueOnOpen = false
        showingQueue = queue
        if (isDestroyed) return
        renderList()
        if (queue) queueFollower.revealCurrent()
    }
    private fun onQueueChanged(itemCount: Int) {
        // The session can connect before its saved queue has loaded. An initial empty
        // timeline is not a final decision; follow its first nonempty update only once.
        if (preferQueueOnOpen && itemCount > 0) {
            preferQueueOnOpen = false
            showingQueue = true
        }
        renderList()
    ...
    private fun showSources() {
        ...
        if (library.sources.isNotEmpty()) {
            ...
            library.sources.forEach { source ->
                ...
                row.addView(TouchButton(this, displayTitle).apply { maxLines=2; setOnClickListener { selectedSource=source.id;showList(false);dialog.dismiss() } },LinearLayout.LayoutParams(-1,-2))
            ...
            action(content,"查看全部音乐") { selectedSource=null;showList(false);dialog.dismiss() }
        }
        ...
        fun submit(existing: String? = null, asIndependent: Boolean = independent) {
            app.scope.launch {
                try {
                    ...
                    if (!isDestroyed) { selectedSource = registered.id; showList(false) }
                } catch (e: CancellationException) { throw e }
    ...
    private fun populateSettings(dialog: AlertDialog, content: LinearLayout) {
        ...
        action(content, "底部留白 · ${BottomClearance.label(this)}") { dialog.dismiss(); showBottomClearance() }
    ...
    private fun showBottomClearance() {
        val (dialog, content) = panel("底部留白 · ${BottomClearance.label(this)}")
        paragraph(content, "调整应用窗口底部的避让高度，为原车底栏留出空间。选择后立即生效并保存，自动可恢复默认窗口。")
        val choices = mutableListOf<Pair<Int, TouchButton>>()
        val columns = ((minOf(640, resources.configuration.screenWidthDp - 32) - 48 + 8) / 104).coerceIn(1, 5)
        BottomClearance.presets.chunked(columns).forEach { rowChoices ->
            val row = LinearLayout(this)
            rowChoices.forEach { (height, label) ->
                val button = TouchButton(this, label).apply {
                    selectedState(height == BottomClearance.selected(this@MainActivity))
                    setOnClickListener {
                        BottomClearance.select(this@MainActivity, height)
                        refreshAppearance(false)
                        choices.forEach { (value, view) -> view.selectedState(value == height) }
                        panels[dialog]?.heading?.text = "底部留白 · ${BottomClearance.label(this@MainActivity)}"
                        Design.styleDialog(dialog, this@MainActivity)
                    }
                }
                choices += height to button
                row.addView(button, LinearLayout.LayoutParams(0, -2, 1f).apply { if(row.childCount>0)marginStart=dp(8) })
            }
            repeat(columns-rowChoices.size) { row.addView(View(this), LinearLayout.LayoutParams(0, 1, 1f).apply { marginStart=dp(8) }) }
    ...
    private fun showSteeringSettings() {
        ...
        toggle(content, "诊断模式（不执行映射）", SteeringSettings.DETECT_ONLY, false)
        paragraph(content, "请先开启车机适配。诊断模式记录按键和菜单，不执行映射，也不阻断原有系统媒体键。")
        ...
        val reconnect = action(content, "重新连接车机按键") {
            if (settings.enabled) app.preferences.edit().putLong(SteeringSettings.RECONNECT, android.os.SystemClock.elapsedRealtime()).apply()
        }.apply { isEnabled = settings.enabled }
        action(content, "复制诊断信息") {
            val version = packageManager.getPackageInfo(packageName, 0).versionName.orEmpty()
            val report = "车机适配：${settings.enabled}；诊断模式：${settings.detectOnly}\n" +
                app.steeringStatus.value.diagnosticReport(version, Build.VERSION.SDK_INT)
            getSystemService(android.content.ClipboardManager::class.java).setPrimaryClip(android.content.ClipData.newPlainText("Cruise Tune 按键诊断", report))
            toast("诊断已复制，可粘贴发送")
        }
    ...
    private fun showDeviceInfo() {
        ...
        paragraph(content, "应用窗口范围：(${origin[0]}, ${origin[1]}) 到 (${origin[0] + decor.width}, ${origin[1] + decor.height}) px\n底部避让：${BottomClearance.label(this)} · ${BottomClearance.selected(this)} dp")
        showPanel(dialog)
    }
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        ...
        outState.putBoolean("preferQueueOnOpen", preferQueueOnOpen)
