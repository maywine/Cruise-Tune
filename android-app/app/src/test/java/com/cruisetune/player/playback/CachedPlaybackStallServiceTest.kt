...
class CachedPlaybackStallServiceTest {
    ...
    private fun fixture(complete: Boolean = true, online: Boolean = true, freshReadGate: CountDownLatch? = null): Fixture {
        ...
        player.play()
        // Do not let a delayed real play acknowledgement overwrite injected stall states.
        await { ReflectionHelpers.getField<Int>(player, "pendingOperationAcks") == 0 }
    ...
    private fun reportEngineState(fixture: Fixture, state: Int) {
    ...
    private fun stalledTick(fixture: Fixture, state: Int) {
        reportEngineState(fixture, state)
    ...
    @Test fun fullCacheFlappingWithoutErrorsStartsOneFreshReadAndThenSkipsOnFailure() {
        ...
        val fixture = fixture(freshReadGate = readGate)
        try {
            ...
            // Finish synthetic READY/BUFFERING injection before the real decoder reports EOF.
            await { ReflectionHelpers.getField<Int>(fixture.player, "pendingOperationAcks") == 0 }
            readGate.countDown()
        ...
        } finally { readGate.countDown(); fixture.service.destroy() }
    }
    ...
    @Test fun nextAndPreviousDoNotCarryTheOriginalSongsStallDeadline() {
        val gate = CountDownLatch(1)
        val fixture = fixture(freshReadGate = gate)
        fun seekWhileBuffering(index: Int) {
            // This fixture injects engine reports without preparing its intentionally invalid audio.
            // An actual seek ACK restores the engine's IDLE state. Keep the next seek's precondition
            // BUFFERING, and drain both seeks before injecting the next synthetic stall timeline.
            await { ReflectionHelpers.getField<Int>(fixture.player, "pendingOperationAcks") == 0 }
            reportEngineState(fixture, Player.STATE_BUFFERING)
            fixture.player.seekTo(index, 0)
            await { ReflectionHelpers.getField<Int>(fixture.player, "pendingOperationAcks") == 0 }
            assertEquals(index, fixture.player.currentMediaItemIndex)
            assertNull(fixture.player.playerError)
            assertEquals("Seeking a buffering item must not start a cache bypass", 0, fixture.freshReads.get())
        }
        try {
            repeat(16) { stalledTick(fixture, Player.STATE_BUFFERING) }
            seekWhileBuffering(1)
            seekWhileBuffering(0)
            repeat(16) { stalledTick(fixture, Player.STATE_BUFFERING) }
            assertEquals(0, fixture.freshReads.get())
