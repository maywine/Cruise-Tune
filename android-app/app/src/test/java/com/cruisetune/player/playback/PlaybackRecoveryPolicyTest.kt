...
class PlaybackRecoveryPolicyTest {
    ...
    @Test fun progressTimeoutsAreRecognizedWithoutCallingThemFileCorruption() {
        for (type in listOf(StuckPlayerException.STUCK_PLAYING_NO_PROGRESS, StuckPlayerException.STUCK_BUFFERING_NO_PROGRESS,
            ...
            val error = StuckPlayerException(type, 10000)
            assertTrue(PlaybackRecoveryPolicy.isProgressFailure(error))
            assertFalse(PlaybackRecoveryPolicy.isFileFailure(error))
        }
        assertFalse(PlaybackRecoveryPolicy.isProgressFailure(StuckPlayerException(StuckPlayerException.STUCK_SUPPRESSED, 10000)))
        assertFalse(PlaybackRecoveryPolicy.isProgressFailure(SocketTimeoutException()))
    }
    @Test fun nativeFlacFrameErrorsRemainDistinctFromNetworkAndAuthenticationErrors() {
        val type = Class.forName("androidx.media3.decoder.flac.FlacDecoderJni\$FlacFrameDecodeException")
        val failure = type.getDeclaredConstructor(String::class.java, Int::class.javaPrimitiveType)
            .apply { isAccessible = true }.newInstance("Synthetic FLAC error", -1) as Exception
        assertTrue(PlaybackRecoveryPolicy.isFileFailure(IOException(failure)))
        failure.initCause(SocketTimeoutException())
        assertFalse(PlaybackRecoveryPolicy.isFileFailure(IOException(failure)))
    }
}
