...
class OneOsConnectionTest {
    ...
    private class ContextFixture(base: Context) : ContextWrapper(base) {
        ...
        override fun bindService(intent: Intent, connection: ServiceConnection, flags: Int): Boolean {
            ...
            assertEquals(OneOsContract.PACKAGE, intent.component!!.packageName)
    ...
    private fun installService(system: Boolean = true) {
        val info = ApplicationInfo().apply { packageName = OneOsContract.PACKAGE; uid = Process.myUid(); if (system) flags = ApplicationInfo.FLAG_SYSTEM }
        val service = ServiceInfo().apply {
            name = OneOsContract.PACKAGE + ".OneOSApiService"; packageName = OneOsContract.PACKAGE
            applicationInfo = info; exported = true; enabled = true
        }
        shadowOf(app.packageManager).installPackage(PackageInfo().apply { packageName = OneOsContract.PACKAGE; applicationInfo = info; services = arrayOf(service) })
    }
    ...
    @Test fun absentServiceDoesNotBindOrPollForever() {
        ...
        var status = ""
        val connection = OneOsConnection(context, scope, { _, value, _ -> status = value }, {}) { _ -> }
        try {
    ...
    @Test fun disconnectRebindsOnceAndOldCallbackCannotDeliverAfterStop() {
        ...
        val client = OneOsConnection(context, scope, { active, _, _ -> connected = active }, {}) { _ -> events++ }
        ...
        try {
            ...
            try {
                data.writeInterfaceToken(OneOsContract.LISTENER); data.writeInt(200087); data.writeInt(0)
                old.transact(2, data, reply, 0)
