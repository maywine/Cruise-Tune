...
- 可选 OneOS 方向盘按键适配：媒体菜单检查、单击／双击／长按映射和检测模式；默认关闭。
- 可选原车仪表媒体显示同步：接收器检查、播放状态与进度同步、暂停清理和脱敏诊断。
...
- FLAC 默认使用 libFLAC 1.5.0 软件解码，支持 ARM 32／64 位。
- 横竖屏布局、日／夜模式、对比度及减少动态效果选项；手动底部留白同时调整窗口、弹窗遮罩和触摸范围。
...
需要 JDK 17、Android SDK Platform 36、Build Tools 35.0.0、NDK 27.2.12479018 和 CMake 3.22.1。Gradle Wrapper 已包含在仓库中；FLAC 模块首次构建会下载固定版本源码，详见 [模块说明](android-app/decoder-flac/README.md)。
